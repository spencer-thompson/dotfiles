...
local runtime = { phone = false, dnd = false, allowed = {} }
...
local function publish()
  metadata:set(0, "state", "Spa:String:JSON", Json.Object {
    ...
    allowed = Json.Object(runtime.allowed),
  }:to_string())
end
...
SimpleEventHook {
  ...
  execute = function(event)
    ...
    metadata:set(tonumber(props["node.id"]), "decision", "Spa:String:JSON", Json.Object {
      app = policy.identity(p), blocked = reason ~= nil, reason = reason or "Allowed",
    }:to_string())
...
SimpleEventHook {
  ...
  execute = function(event)
    ...
    elseif action == "reset" then
      choice, runtime = policy.defaults(), { phone = false, dnd = false, allowed = {} }
      persist = true
